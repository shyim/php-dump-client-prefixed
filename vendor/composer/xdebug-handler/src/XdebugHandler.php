 ...
 */
namespace _PhpScoper3fe455fa007d\Composer\XdebugHandler;
...
class XdebugHandler
{
    ...
    public function __construct($envPrefix, $colorOption = '')
    {
        ...
        $this->statusWriter = new \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
    ...
    {
        $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::CHECK, $this->loaded);
        $envArgs = \explode('|', (string) \getenv($this->envAllowXdebug));
        if (empty($envArgs[0]) && $this->requiresRestart((bool) $this->loaded)) {
            // Restart required
            $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && \count($envArgs) === 5) {
            ...
            $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::RESTARTED);
            \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
            if (!$this->loaded) {
        ...
        $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::NORESTART);
    ...
    {
        $this->tryEnableSignals();
        $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::RESTARTING, $command);
        ...
        if (!isset($exitCode)) {
            ...
            $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::ERROR, 'Unable to restart process');
            $exitCode = -1;
        } else {
            $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::INFO, 'Restarted process exited ' . $exitCode);
        }
        if ($this->debug === '2') {
            $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::INFO, 'Temp ini saved: ' . $this->tmpIni);
        } else {
            @\unlink($this->tmpIni);
        }
        exit($exitCode);
    ...
    private function prepareRestart()
    {
        ...
        if ($error) {
            $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return empty($error);
    }
    /**
    ...
    private function getCommand()
    {
        ...
        if (\defined('STDOUT') && \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::supportsColor(\STDOUT)) {
            $args = \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::addColorOption($args, $this->colorOption);
        }
        $args = \array_merge($php, array($this->script), $args);
        $cmd = \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::escape(\array_shift($args), \true, \true);
        foreach ($args as $arg) {
            $cmd .= ' ' . \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::escape($arg);
        }
        return $cmd;
    }
    /**
    ...
    private function setEnvRestartSettings($envArgs)
    {
        ...
        \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, \implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings)
    {
        if (\false === \getenv($this->envOriginalInis)) {
            ...
            \_PhpScoper3fe455fa007d\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, \implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
    ...
    private function tryEnableSignals()
    {
        ...
        $this->notify(\_PhpScoper3fe455fa007d\Composer\XdebugHandler\Status::INFO, $message);
    }
}
