 ...
 * @author SpacePossum
 */
final class ReturnAssignmentFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * @var TokensAnalyzer
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Local, dynamic and directly referenced variables should not be assigned and directly returned by a function or method.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php\nfunction a() {\n    \$a = 1;\n    return \$a;\n}\n")]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        for ($index = 1; $index < $tokenCount; ++$index) {
            ...
            $functionCloseIndex = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $functionOpenIndex);
            $totalTokensAdded = 0;
            do {
     ...
     */
    private function fixFunction(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $functionIndex, $functionOpenIndex, $functionCloseIndex)
    {
        static $riskyKinds = [
        ...
        for ($index = $functionOpenIndex + 1; $index < $functionCloseIndex; ++$index) {
            if ($tokens[$index]->isGivenKind(\T_FUNCTION)) {
                ...
                $nestedFunctionCloseIndex = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $nestedFunctionOpenIndex);
            ...
            if ($isRisky) {
                continue;
                // don't bother to look into anything else than nested functions as the current is risky already
            }
            if ($tokens[$index]->equals('&')) {
                $isRisky = \true;
                continue;
        ...
        for ($i = \count($candidates) - 1; $i >= 0; --$i) {
            ...
            do {
                ...
                $assignVarEndIndex = $tokens->findBlockStart(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $prevMeaningFul);
            } while (\true);
            ...
            // Note: here we are @ "[;{}] $a = [^;{<? ? >] ; return $a;"
            $inserted += $this->simplifyReturnStatement($tokens, $assignVarIndex, $assignVarOperatorIndex, $index, $endReturnVarIndex);
        }
        return $inserted;
    }
    /**
     ...
     * @return int >= 0 number of tokens inserted into the Tokens collection
     */
    private function simplifyReturnStatement(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $assignVarIndex, $assignVarOperatorIndex, $returnIndex, $returnVarEndIndex)
    {
        $inserted = 0;
        ...
        if ($tokens[$returnIndex - 1]->isWhitespace()) {
            ...
            $tokens[$returnIndex - 1] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $content]);
        }
        ...
        $tokens->insertAt($assignVarIndex, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_RETURN, 'return']));
        ++$inserted;
        ...
        if (null !== $originalIndent && $tokens[$assignVarIndex - 1]->isWhitespace() && $originalIndent !== $tokens[$assignVarIndex - 1]->getContent()) {
            $tokens[$assignVarIndex - 1] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $originalIndent]);
        }
        ...
        if (!$tokens[$nextIndex]->isWhitespace()) {
            $tokens->insertAt($nextIndex, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']));
            ++$inserted;
        }
        return $inserted;
    }
    private function clearIfSave(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
