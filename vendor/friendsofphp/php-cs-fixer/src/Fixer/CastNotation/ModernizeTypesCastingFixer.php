 ...
 */
final class ModernizeTypesCastingFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFunctionReferenceFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Replaces `intval`, `floatval`, `doubleval`, `strval` and `boolval` function calls with according type casting operator.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
    $a = intval($b);
    $a = floatval($b);
    $a = doubleval($b);
    $a = strval ($b);
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        foreach ($replacement as $functionIdentity => $newToken) {
            ...
            while (null !== $currIndex) {
                ...
                $replacementSequence = [new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token($newToken), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' '])];
                if ($wrapInParenthesises) {
                    \array_unshift($replacementSequence, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('('));
                }
                ...
                if ($wrapInParenthesises) {
                    $tokens->insertAt($closeParenthesis, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(')'));
                }
