 ...
 */
namespace _PhpScoper3fe455fa007d\PhpCsFixer\Fixer\Naming;
 ...
 */
final class NoHomoglyphNamesFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Replace accidental usage of homoglyphs (non ascii characters) in names.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php \$nаmе = 'wrong \"a\" character';\n")], null, 'Renames classes and cannot rename the files. You might have string references to renamed code (`$$name`).');
    }
    /**
     * {@inheritdoc}
     */
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        foreach ($tokens as $index => $token) {
            ...
            $replaced = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replaceCallback('/[^[:ascii:]]/u', static function ($matches) {
                return isset(self::$replacements[$matches[0]]) ? self::$replacements[$matches[0]] : $matches[0];
            }, $token->getContent(), -1, $count);
            if ($count) {
                $tokens->offsetSet($index, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([$token->getId(), $replaced]));
            }
        }
    }
}
