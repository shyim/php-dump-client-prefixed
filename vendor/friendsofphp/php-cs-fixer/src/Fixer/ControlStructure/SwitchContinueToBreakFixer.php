...
final class SwitchContinueToBreakFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Switch case must not be ended with `continue` but with `break`.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
switch ($foo) {
    case 1:
        continue;
}
...
')]);
     ...
     * @return int
     */
    private function doFix(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $depth, $isInSwitch)
    {
        $token = $tokens[$index];
     ...
     * @return int
     */
    private function fixInSwitch(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $switchIndex, $depth)
    {
     ...
     * @return int
     */
    private function fixInLoop(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $openIndex, $depth)
    {
        $openCount = 1;
        do {
            ++$openIndex;
     ...
     * @return int
     */
    private function fixContinueWhenActsAsBreak(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $continueIndex, $isInSwitch, $depth)
    {
        ...
        if (\strlen($jump) > 2 && 'x' === $jump[1]) {
            ...
            $jump = \octdec($jump);
            // octal 01
        } elseif (1 === \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match('#^\\d+$#', $jump)) {
            // positive int
            $jump = (float) $jump;
     ...
     * @param int $index
     */
    private function replaceContinueWithBreakToken(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $tokens[$index] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_BREAK, 'break']);
    }
}
