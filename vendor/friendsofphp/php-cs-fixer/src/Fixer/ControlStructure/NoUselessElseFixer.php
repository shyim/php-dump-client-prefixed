 ...
 * @author SpacePossum
 */
final class NoUselessElseFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractNoUselessElseFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('There should not be useless `else` cases.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php\nif (\$a) {\n    return 1;\n} else {\n    return 2;\n}\n")]);
    }
    /**
     * {@inheritdoc}
     *
     ...
     */
    private function fixEmptyElse(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $next = $tokens->getNextMeaningfulToken($index);
        if ($tokens[$next]->equals('{')) {
            $close = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $next);
            if (1 === $close - $next) {
                // '{}'
                $this->clearElse($tokens, $index);
     ...
     */
    private function clearElse(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        $tokens->clearTokenAndMergeSurroundingWhitespace($tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $next));
