...
final class SelfStaticAccessorFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * @var TokensAnalyzer
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Inside a `final` class or anonymous class `self` should be preferred to `static`.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
final class Sample
{
    private static $A = 1;
...
', new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000))]);
    ...
    {
        $this->tokensAnalyzer = $tokensAnalyzer = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\TokensAnalyzer($tokens);
     ...
     * @return int
     */
    private function fixClass(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        while ($classOpenCount > 0) {
            ...
            if ($tokens[$index]->isGivenKind([\T_NEW, \T_INSTANCEOF])) {
                ...
                if ($tokens[$index]->isGivenKind(\T_STATIC)) {
                    $tokens[$index] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'self']);
                }
                continue;
            }
            if (!$tokens[$index]->isGivenKind(\T_STATIC)) {
            ...
            $tokens[$staticIndex] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'self']);
        }
        return $index;
    }
}
