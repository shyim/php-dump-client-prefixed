 ...
 */
final class NoPhp4ConstructorFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Convert PHP4-style constructors to `__construct`.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
class Foo
{
...
')], null, 'Risky when old style constructor being fixed is overridden or overrides parent one.');
        ...
        for ($i = 0; $i < $numClasses; ++$i) {
            ...
            if (null !== $nspIndex) {
                ...
                if (!$tokens[$nspIndex]->equals('{')) {
                    ...
                    $nspEnd = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $nspIndex);
                    if ($index < $nspEnd) {
     ...
     */
    private function fixConstructor(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $className, $classStart, $classEnd)
    {
        ...
        if (null === $php5) {
            ...
            $tokens[$php4['nameIndex']] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, '__construct']);
     ...
     * @param int    $classEnd   the class end index
     */
    private function fixParent(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $classStart, $classEnd)
    {
        ...
        foreach ($tokens->findGivenKind(\T_EXTENDS) as $index => $token) {
            ...
            if (null !== $parentSeq) {
                ...
                if ($tokens[$parentSeq[0]]->equalsAny([[\T_STRING, 'parent'], [\T_STRING, $parentClass]], \false)) {
                    ...
                    $tokens[$parentSeq[0]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'parent']);
                    $tokens[$parentSeq[2]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, '__construct']);
                }
            }
            ...
            if (null !== $parentSeq) {
                ...
                $tokens[$parentSeq[0]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'parent']);
                $tokens[$parentSeq[1]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_DOUBLE_COLON, '::']);
                $tokens[$parentSeq[2]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, '__construct']);
            }
        }
    }
     ...
     */
    private function fixInfiniteRecursion(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $start, $end)
    {
        ...
        while (\true) {
            ...
            $tokens[$callSeq[0]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'parent']);
            $tokens[$callSeq[1]] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_DOUBLE_COLON, '::']);
        }
    }
    /**
     ...
     */
    private function getWrapperMethodSequence(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $method, $startIndex, $bodyIndex)
    {
     ...
     */
    private function findFunction(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $name, $startIndex, $endIndex)
    {
        ...
        if (isset($modifiers[\T_ABSTRACT])) {
            ...
            $funcEnd = null !== $bodyStart ? $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $bodyStart) : null;
        }
