...
use _PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\ReferenceAnalyzer;
 ...
 */
final class OperatorLinebreakFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * @internal
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Operators - when multiline - must always be at the beginning or at the end of the line.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
function foo() {
    return $bar ||
        $baz;
}
...
', ['position' => 'end'])]);
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('only_booleans', 'whether to limit operators to only boolean ones'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption(), (new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('position', 'whether to place operators at the beginning or at the end of the line'))->setAllowedValues(['beginning', 'end'])->setDefault($this->position)->getOption()]);
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        $referenceAnalyzer = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\ReferenceAnalyzer();
     ...
     * @return int[]
     */
    private function getExcludedIndices(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $indices = [];
     ...
     * @return int[]
     */
    private function getCasesColonsForSwitch(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $switchIndex)
    {
     ...
     */
    private function fixOperatorLinebreak(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, array $operatorIndices)
    {
        /** @var int $prevIndex */
     ...
     * @param int[] $operatorIndices
     */
    private function fixMoveToTheBeginning(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, array $operatorIndices)
    {
        /** @var int $prevIndex */
        ...
        for ($i = $nextIndex - 1; $i > \max($operatorIndices); --$i) {
            if ($tokens[$i]->isWhitespace() && 1 === \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match('/\\R/u', $tokens[$i]->getContent())) {
                ...
                if ($isWhitespaceBefore) {
                    $inserts[] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']);
                }
     ...
     * @param int[] $operatorIndices
     */
    private function fixMoveToTheEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, array $operatorIndices)
    {
        /** @var int $prevIndex */
        $prevIndex = $tokens->getPrevMeaningfulToken(\min($operatorIndices));
        /** @var int $nextIndex */
        ...
        for ($i = $prevIndex + 1; $i < \max($operatorIndices); ++$i) {
            if ($tokens[$i]->isWhitespace() && 1 === \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match('/\\R/u', $tokens[$i]->getContent())) {
                ...
                if ($isWhitespaceAfter) {
                    \array_unshift($inserts, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']));
                }
     ...
     * @return Token[]
     */
    private function getReplacementsAndClear(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, array $indices, $direction)
    {
     ...
     * @return bool
     */
    private function isMultiline(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $indexStart, $indexEnd)
    {
