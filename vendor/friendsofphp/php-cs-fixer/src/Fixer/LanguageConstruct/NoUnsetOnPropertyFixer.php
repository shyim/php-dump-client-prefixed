 ...
 */
final class NoUnsetOnPropertyFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
     ...
     */
    private function getUnsetsInfo(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        $unsetEnd = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $unsetStart);
        $isFirst = \true;
        $unsets = [];
     ...
     * @return bool
     */
    private function isProperty(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $endIndex)
    {
     ...
     */
    private function updateTokens(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, array $unsetInfo, $isLastUnset)
    {
        ...
        if (!$unsetInfo['isToTransform'] && !$isLastUnset) {
            $tokens->insertAt($unsetInfo['endIndex'] + 1, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(')'));
        }
        // if entry is to be unset and is not first we add leading "unset("
        if (!$unsetInfo['isToTransform'] && !$unsetInfo['isFirst']) {
            $tokens->insertAt($unsetInfo['startIndex'], [new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_UNSET, 'unset']), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('(')]);
        }
        // and finally
        // if entry is to be transformed we add trailing " = null"
        if ($unsetInfo['isToTransform']) {
            $tokens->insertAt($unsetInfo['endIndex'] + 1, [new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('='), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'null'])]);
        }
    }
}
