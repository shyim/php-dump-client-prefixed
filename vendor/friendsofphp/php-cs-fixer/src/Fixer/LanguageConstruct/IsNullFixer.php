 ...
 */
final class IsNullFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Replaces `is_null($var)` expression with `null === $var`.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php\n\$a = is_null(\$b);\n")], null, 'Risky when the function `is_null` is overridden.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        while (null !== $currIndex) {
            ...
            $referenceEnd = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $matches[1]);
            ...
            for ($paramTokenIndex = $matches[1]; $paramTokenIndex <= $referenceEnd; ++$paramTokenIndex) {
                if (\in_array($tokens[$paramTokenIndex]->getContent(), ['?', '?:', '=', '??'], \true)) {
                    $isContainingDangerousConstructs = \true;
                    break;
                }
            }
            ...
            $replacement = [new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, 'null']), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token($isInvertedNullCheck ? [\T_IS_NOT_IDENTICAL, '!=='] : [\T_IS_IDENTICAL, '===']), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' '])];
            if (\true === $this->configuration['use_yoda_style']) {
                if ($wrapIntoParentheses) {
                    \array_unshift($replacement, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('('));
                    $tokens->insertAt($referenceEnd + 1, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(')'));
                }
                $tokens->overrideRange($isNullIndex, $isNullIndex, $replacement);
            } else {
                $replacement = \array_reverse($replacement);
                if ($wrapIntoParentheses) {
                    $replacement[] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(')');
                    $tokens[$isNullIndex] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('(');
                } else {
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('use_yoda_style', 'Whether Yoda style conditions should be used.'))->setAllowedTypes(['bool'])->setDefault(\true)->setDeprecationMessage('Use `yoda_style` fixer instead.')->getOption()]);
    }
}
