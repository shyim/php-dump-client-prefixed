 ...
 * @author SpacePossum
 */
final class PowToExponentiationFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFunctionReferenceFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Converts `pow` to the `**` operator.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php\n pow(\$a, 1);\n")], null, 'Risky when the function `pow` is overridden.');
    }
    /**
     * {@inheritdoc}
     *
     ...
     * @return array[]
     */
    private function findPowCalls(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $candidates = [];
        // Minimal candidate to fix is seven tokens: pow(x,y);
        $end = \count($tokens) - 6;
     ...
     */
    private function fixPowToExponentiation(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $functionNameIndex, $openParenthesisIndex, $closeParenthesisIndex, array $arguments)
    {
        // find the argument separator ',' directly after the last token of the first argument;
        // replace it with T_POW '**'
        $tokens[$tokens->getNextTokenOfKind(\reset($arguments), [','])] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_POW, '**']);
        ...
        foreach (\array_reverse($arguments, \true) as $argumentStartIndex => $argumentEndIndex) {
            if ($this->isParenthesisNeeded($tokens, $argumentStartIndex, $argumentEndIndex)) {
                $tokens->insertAt($argumentEndIndex + 1, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(')'));
                $tokens->insertAt($argumentStartIndex, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('('));
                $added += 2;
            }
        }
     ...
     * @return bool
     */
    private function isParenthesisNeeded(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $argumentStartIndex, $argumentEndIndex)
    {
        static $allowedKinds = [\T_DNUMBER, \T_LNUMBER, \T_VARIABLE, \T_STRING, \T_OBJECT_OPERATOR, \T_CONSTANT_ENCAPSED_STRING, \T_DOUBLE_CAST, \T_INT_CAST, \T_INC, \T_DEC, \T_NS_SEPARATOR, \T_WHITESPACE, \T_DOUBLE_COLON, \T_LINE, \T_COMMENT, \T_DOC_COMMENT, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR];
        for ($i = $argumentStartIndex; $i <= $argumentEndIndex; ++$i) {
            ...
            if ($tokens[$i]->equals('$')) {
                ...
                if ($tokens[$i]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_VAR_BRACE_OPEN)) {
                    $i = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_DYNAMIC_VAR_BRACE, $i);
                    continue;
                }
            }
