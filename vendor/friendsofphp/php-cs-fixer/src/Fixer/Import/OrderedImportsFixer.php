 ...
 */
final class OrderedImportsFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface, \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Ordering `use` statements.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php\nuse Z; use A;\n"), new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
use Acme\\Bar;
use Bar1;
use Acme;
use Bar;
', ['sort_algorithm' => self::SORT_LENGTH]), new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample("<?php\nuse function AAC;\nuse const AAB;\nuse AAA;\n", new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000)), new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
use const AAAA;
use const BBB;
...
use function CCC\\AA;
use function DDD;
', new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000), ['sort_algorithm' => self::SORT_LENGTH, 'imports_order' => [self::IMPORT_TYPE_CONST, self::IMPORT_TYPE_CLASS, self::IMPORT_TYPE_FUNCTION]]), new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
use const BBB;
use const AAAA;
...
use function DDD;
use function CCC\\AA;
', new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000), ['sort_algorithm' => self::SORT_ALPHA, 'imports_order' => [self::IMPORT_TYPE_CONST, self::IMPORT_TYPE_CLASS, self::IMPORT_TYPE_FUNCTION]]), new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
use const BBB;
use const AAAA;
...
use AAC;
use Bar;
', new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000), ['sort_algorithm' => self::SORT_NONE, 'imports_order' => [self::IMPORT_TYPE_CONST, self::IMPORT_TYPE_CLASS, self::IMPORT_TYPE_FUNCTION]])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        foreach ($usesOrder as $index => $use) {
            $declarationTokens = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::fromCode(\sprintf('<?php use %s%s;', self::IMPORT_TYPE_CLASS === $use['importType'] ? '' : ' ' . $use['importType'] . ' ', $use['namespace']));
            ...
            if ($use['group']) {
                ...
                if ($tokens[$prev]->equals(',')) {
                    $tokens[$prev] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(';');
                    $tokens->insertAt($prev + 1, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_USE, 'use']));
                    if (!$tokens[$prev + 2]->isWhitespace()) {
                        $tokens->insertAt($prev + 2, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']));
                    }
                }
            }
        }
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\AliasedFixerOptionBuilder(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('sort_algorithm', 'whether the statements should be sorted alphabetically or by length, or not sorted'), 'sortAlgorithm'))->setAllowedValues($this->supportedSortAlgorithms)->setDefault(self::SORT_ALPHA)->getOption(), (new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\AliasedFixerOptionBuilder(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('imports_order', 'Defines the order of import types.'), 'importsOrder'))->setAllowedTypes(['array', 'null'])->setAllowedValues([static function ($value) use($supportedSortTypes) {
            if (null !== $value) {
                $missing = \array_diff($supportedSortTypes, $value);
                if (\count($missing)) {
                    throw new \_PhpScoper3fe455fa007d\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Missing sort %s "%s".', 1 === \count($missing) ? 'type' : 'types', \implode('", "', $missing)));
                }
                $unknown = \array_diff($value, $supportedSortTypes);
                if (\count($unknown)) {
                    throw new \_PhpScoper3fe455fa007d\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Unknown sort %s "%s".', 1 === \count($unknown) ? 'type' : 'types', \implode('", "', $unknown)));
                }
            }
            return \true;
    ...
    {
        return \trim(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('%/\\*(.*)\\*/%s', '', $namespace));
    }
    /**
     * @param int[] $uses
     *
     * @return array
     */
    private function getNewOrder(array $uses, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $indexes = [];
        $originalIndexes = [];
        ...
        for ($i = \count($uses) - 1; $i >= 0; --$i) {
            ...
            $group = $tokens[$previous]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_CLOSE);
            if ($tokens[$startIndex]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_CONST_IMPORT)) {
                $type = self::IMPORT_TYPE_CONST;
                $index = $tokens->getNextNonWhitespace($startIndex);
            } elseif ($tokens[$startIndex]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_FUNCTION_IMPORT)) {
                ...
                if ($index === $endIndex || !$group && $token->equals(',')) {
                    if ($group && self::SORT_NONE !== $this->configuration['sort_algorithm']) {
                        ...
                        for ($k = 0; $k < $namespaceTokensCount; ++$k) {
                            if ($namespaceTokens[$k]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_OPEN)) {
                                $namespace .= '{';
                                break;
                            }
                        ...
                        for ($k1 = $k + 1; $k1 < $namespaceTokensCount; ++$k1) {
                            ...
                            $namespacePart = '';
                            for ($k2 = $k1;; ++$k2) {
                                if ($namespaceTokens[$k2]->equalsAny([',', [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_CLOSE]])) {
                                    break;
                                }
                        ...
                        if ($sortedParts === $parts) {
                            $namespace = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::fromArray($namespaceTokens)->generateCode();
                        } else {
