 ...
 */
final class EscapeImplicitBackslashesFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * {@inheritdoc}
     */
...
EOF;
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            $isDoubleQuotedString = $token->isGivenKind(\T_CONSTANT_ENCAPSED_STRING) && ('"' === $content[0] || 'b"' === $firstTwoCharacters) || $token->isGivenKind(\T_ENCAPSED_AND_WHITESPACE) && $doubleQuoteOpened;
            $isHeredocSyntax = !$isSingleQuotedString && !$isDoubleQuotedString;
            if (\false === $this->configuration['single_quoted'] && $isSingleQuotedString || \false === $this->configuration['double_quoted'] && $isDoubleQuotedString || \false === $this->configuration['heredoc_syntax'] && $isHeredocSyntax) {
                continue;
            }
            ...
            $newContent = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace($regex, '\\\\\\\\$1', $content);
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('single_quoted', 'Whether to fix single-quoted strings.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption(), (new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('double_quoted', 'Whether to fix double-quoted strings.'))->setAllowedTypes(['bool'])->setDefault(\true)->getOption(), (new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('heredoc_syntax', 'Whether to fix heredoc syntax.'))->setAllowedTypes(['bool'])->setDefault(\true)->getOption()]);
    }
}
