...
use _PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FileSpecificCodeSample;
 ...
 */
final class PsrAutoloadingFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Classes must be in a path that matches their namespace, be at least one namespace deep and the class name should match the file name.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FileSpecificCodeSample('<?php
namespace PhpCsFixer\\FIXER\\Basic;
class InvalidName {}
', new \SplFileInfo(__FILE__)), new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FileSpecificCodeSample('<?php
namespace PhpCsFixer\\FIXER\\Basic;
class InvalidName {}
', new \SplFileInfo(__FILE__), ['dir' => './src'])], null, 'This fixer may change your class name, which will break the code that depends on the old name.');
    ...
    {
        if ($file instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\StdinFileInfo) {
            return \false;
        }
        if ('php' !== $file->getExtension() || 0 === \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $file->getBasename('.php'))) {
            return \false;
        }
        try {
            $tokens = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::fromCode(\sprintf('<?php class %s {}', $file->getBasename('.php')));
        ...
        return !\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match('{[/\\\\](stub|fixture)s?[/\\\\]}i', $file->getRealPath());
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('dir', 'If provided, the directory where the project code is placed.'))->setAllowedTypes(['null', 'string'])->setDefault(null)->getOption()]);
    }
    /**
     * {@inheritdoc}
     */
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        if ($classyName !== $expectedClassyName) {
            $tokens[$classyIndex] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_STRING, $expectedClassyName]);
        }
        ...
        if ($originalNamespace !== $newNamespace && \strtolower($originalNamespace) === \strtolower($newNamespace)) {
            ...
            $newNamespace = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::fromCode('<?php namespace ' . $namespace . ';');
