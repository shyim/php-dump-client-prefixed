 ...
 */
final class NoUnreachableDefaultArgumentValueFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('In function arguments there must not be arguments with default values before non-default ones.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
')], null, 'Modifies the signature of functions; therefore risky when using systems (such as some Symfony components) that rely on those (for example through reflection).');
    ...
    protected function applyFix(\SplFileInfo $file, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        for ($i = 0, $l = $tokens->count(); $i < $l; ++$i) {
            ...
            $i = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $startIndex);
     ...
     * @param int $endIndex
     */
    private function fixFunctionDefinition(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $startIndex, $endIndex)
    {
     ...
     * @return null|int
     */
    private function getLastNonDefaultArgumentIndex(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $startIndex, $endIndex)
    {
     ...
     * @return bool
     */
    private function isEllipsis(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $variableIndex)
    {
     ...
     * @param int $endIndex
     */
    private function removeDefaultArgument(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $startIndex, $endIndex)
    {
     ...
     * @return bool
     */
    private function isNonNullableTypehintedNullableVariable(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        $searchTokens = [',', '(', [\T_STRING], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_ARRAY_TYPEHINT], [\T_CALLABLE]];
        $typehintKinds = [\T_STRING, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_ARRAY_TYPEHINT, \T_CALLABLE];
        $prevIndex = $tokens->getPrevTokenOfKind($variableIndex, $searchTokens);
        if (!$tokens[$prevIndex]->isGivenKind($typehintKinds)) {
            return \false;
        }
        return !$tokens[$tokens->getPrevMeaningfulToken($prevIndex)]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_NULLABLE_TYPE);
    }
    /**
     * @param int $index
     */
    private function clearWhitespacesBeforeIndex(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
