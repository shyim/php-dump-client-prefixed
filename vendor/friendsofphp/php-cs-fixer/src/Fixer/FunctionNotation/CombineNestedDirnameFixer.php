 ...
 */
final class CombineNestedDirnameFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Replace multiple nested calls of `dirname` by only one call with second `$level` parameter. Requires PHP >= 7.0.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample("<?php\ndirname(dirname(dirname(\$path)));\n", new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000))], null, 'Risky when the function `dirname` is overridden.');
    }
    /**
     * {@inheritdoc}
     */
     ...
     */
    private function getDirnameInfo(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $firstArgumentEndIndex = null)
    {
        if (!$tokens[$index]->equals([\T_STRING, 'dirname'], \false)) {
            return \false;
        }
        if (!(new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\FunctionsAnalyzer())->isGlobalFunctionCall($tokens, $index)) {
            return \false;
        }
        $info['indexes'] = [];
        ...
        if (null !== $firstArgumentEndIndex) {
            ...
            while (!$tokens[$next]->equalsAny([',', ')'])) {
                $blockType = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::detectBlockType($tokens[$next]);
                if ($blockType) {
    ...
    private function combineDirnames(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, array $dirnameInfoArray)
    {
        ...
        $levelsToken = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_LNUMBER, (string) $levels]);
        if (isset($outerDirnameInfo['secondArgument'])) {
            ...
            $items = [];
            if (!$tokens[$prev]->equals(',')) {
                $items = [new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(','), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' '])];
            }
            $items[] = $levelsToken;
