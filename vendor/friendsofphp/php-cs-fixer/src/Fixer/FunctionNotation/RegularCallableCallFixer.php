 ...
 */
final class RegularCallableCallFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Callables must be called without using `call_user_func*` when possible.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
', new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecification(70000))], null, 'Risky when the `call_user_func` or `call_user_func_array` function is overridden or when are used in constructions that should be avoided, like `call_user_func_array(\'foo\', [\'bar\' => \'baz\'])` or `call_user_func($foo, $foo = \'bar\')`.');
    }
    /**
     * {@inheritdoc}
     */
     ...
     * @param int $index
     */
    private function processCall(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index, array $arguments)
    {
        ...
        if ($firstArgToken->isGivenKind(\T_CONSTANT_ENCAPSED_STRING)) {
            ...
            $newCallTokens = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::fromCode('<?php ' . \substr($firstArgToken->getContent(), 1, -1) . '();');
            ...
            if (\PHP_VERSION_ID >= 70000) {
                $firstArgEndIndex = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $tokens->getNextTokenOfKind($firstArgIndex, ['{']));
                $newCallTokens = $this->getTokensSubcollection($tokens, $firstArgIndex, $firstArgEndIndex);
                $newCallTokens->insertAt($newCallTokens->count(), new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token(')'));
                $newCallTokens->insertAt(0, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token('('));
            ...
            for ($newCallIndex = \count($newCallTokens) - 1; $newCallIndex >= 0; --$newCallIndex) {
                ...
                $blockType = \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::detectBlockType($newCallTokens[$newCallIndex]);
                if (null !== $blockType && (\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_ARRAY_INDEX_CURLY_BRACE === $blockType['type'] || \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_INDEX_SQUARE_BRACE === $blockType['type'])) {
     ...
     */
    private function replaceCallUserFuncWithCallback(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $callIndex, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $newCallTokens, $firstArgStartIndex, $firstArgEndIndex)
    {
        ...
        if ($afterFirstArgToken->equals(',')) {
            ...
            if ($useEllipsis) {
                ...
                $tokens->insertAt($secondArgIndex, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_ELLIPSIS, '...']));
            }
    ...
    private function getTokensSubcollection(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $indexStart, $indexEnd)
    {
        $size = $indexEnd - $indexStart + 1;
        $subcollection = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens($size);
        for ($i = 0; $i < $size; ++$i) {
            /** @var Token $toClone */
