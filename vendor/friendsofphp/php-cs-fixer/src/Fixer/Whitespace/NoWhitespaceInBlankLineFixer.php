 ...
 */
final class NoWhitespaceInBlankLineFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FixerDefinition('Remove trailing whitespace at the end of blank lines.', [new \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSample("<?php\n   \n\$a = 1;\n")]);
    }
    /**
     * {@inheritdoc}
     *
     ...
     * @param int $index
     */
    private function fixWhitespaceToken(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $content = $tokens[$index]->getContent();
        $lines = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::split("/(\r\n|\n)/", $content);
        ...
        if ($lineCount > 2 || $lineCount > 0 && (!isset($tokens[$index + 1]) || $tokens[$index - 1]->isGivenKind(\T_OPEN_TAG))) {
            ...
            for ($l = $lStart; $l < $lMax; ++$l) {
                $lines[$l] = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/^\\h+$/', '', $lines[$l]);
            }
