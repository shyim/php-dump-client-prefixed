 ...
 * @internal
 */
final class JunitReporter implements \_PhpScoper3fe455fa007d\PhpCsFixer\Report\ReporterInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    public function generate(\_PhpScoper3fe455fa007d\PhpCsFixer\Report\ReportSummary $reportSummary)
    {
        ...
        return $reportSummary->isDecoratedOutput() ? \_PhpScoper3fe455fa007d\Symfony\Component\Console\Formatter\OutputFormatter::escape($dom->saveXML()) : $dom->saveXML();
    }
    ...
    private function createFailedTestCases(\DOMDocument $dom, \DOMElement $testsuite, \_PhpScoper3fe455fa007d\PhpCsFixer\Report\ReportSummary $reportSummary)
    {
        $assertionsCount = 0;
    ...
    private function createFailedTestCase(\DOMDocument $dom, $file, array $fixResult, $shouldAddAppliedFixers)
    {
        ...
        $testName = \str_replace('.', '_DOT_', \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('@\\.' . \pathinfo($file, \PATHINFO_EXTENSION) . '$@', '', $file));
        $testcase = $dom->createElement('testcase');
