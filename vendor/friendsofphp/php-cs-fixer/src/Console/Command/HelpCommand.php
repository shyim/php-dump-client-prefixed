...
use _PhpScoper3fe455fa007d\Symfony\Component\Console\Command\HelpCommand as BaseHelpCommand;
 ...
 * @internal
 */
final class HelpCommand extends \_PhpScoper3fe455fa007d\Symfony\Component\Console\Command\HelpCommand
{
...
EOF;
     ...
     * @return null|array
     */
    public static function getDisplayableAllowedValues(\_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionInterface $option)
    {
        ...
        if (null !== $allowed) {
            ...
            \usort($allowed, static function ($valueA, $valueB) {
                if ($valueA instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\AllowedValueSubset) {
                    return -1;
                }
                if ($valueB instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\AllowedValueSubset) {
                    return 1;
                }
    ...
    public static function getLatestReleaseVersionFromChangeLog()
    {
        ...
        if (null === $changelogFile) {
            $version = \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Application::VERSION;
            return $version;
        }
        ...
        for ($i = \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Application::getMajorVersion(); $i > 0; --$i) {
            if (1 === \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match('/Changelog for v(' . $i . '.\\d+.\\d+)/', $changelog, $matches)) {
                $version = $matches[1];
                break;
            }
        }
    ...
    {
        $output->getFormatter()->setStyle('url', new \_PhpScoper3fe455fa007d\Symfony\Component\Console\Formatter\OutputFormatterStyle('blue'));
    }
    /**
     * @return null|string
     */
    ...
    private static function getFixersHelp()
    {
        ...
        // sort fixers by name
        \usort($fixers, static function (\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $a, \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $b) {
            return \strcmp($a->getName(), $b->getName());
        });
        $ruleSets = [];
        foreach (\_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSets::getSetDefinitionNames() as $setName) {
            $ruleSets[$setName] = new \_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSet([$setName => \true]);
        }
        ...
        foreach ($fixers as $i => $fixer) {
            ...
            $description = \implode("\n   | ", self::wordwrap(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', $description), 72));
            if (!empty($sets)) {
            ...
            if ($fixer->isRisky()) {
                $help .= \sprintf("   | *Risky rule: %s.*\n", \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \lcfirst(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/\\.$/', '', $fixer->getDefinition()->getRiskyDescription()))));
            }
            if ($fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface) {
                ...
                if (\count($configurationDefinitionOptions)) {
                    ...
                    \usort($configurationDefinitionOptions, static function (\_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionInterface $optionA, \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\FixerOptionInterface $optionB) {
                    ...
                    foreach ($configurationDefinitionOptions as $option) {
                        ...
                        $line .= ': ' . \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \lcfirst(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/\\.$/', '', \_PhpScoper3fe455fa007d\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getDescription())))) . '; ';
                        ...
                        if ($option instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\DeprecatedFixerOption) {
                            $line .= '. DEPRECATED: ' . \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \lcfirst(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/\\.$/', '', \_PhpScoper3fe455fa007d\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getDeprecationMessage()))));
                        }
        ...
        return \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('#\\\\(</.*?>)#', '<<$1', $help);
    }
    /**
    ...
    private static function wordwrap($string, $width)
    {
        ...
        foreach (\explode(' ', $string) as $word) {
            $wordLength = \strlen(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('~</?(\\w+)>~', '', $word));
            if (0 !== $lineLength) {
                ++$wordLength;
                // space before word
            }
    ...
    private static function scalarToString($value)
    {
        ...
        return \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/\\bNULL\\b/', 'null', $str);
    }
    /**
     * @return string
     */
