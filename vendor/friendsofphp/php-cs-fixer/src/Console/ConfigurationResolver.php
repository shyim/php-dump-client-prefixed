...
use _PhpScoper3fe455fa007d\PhpCsFixer\Cache\FileCacheManager;
use _PhpScoper3fe455fa007d\PhpCsFixer\Cache\FileHandler;
use _PhpScoper3fe455fa007d\PhpCsFixer\Cache\NullCacheManager;
use _PhpScoper3fe455fa007d\PhpCsFixer\Cache\Signature;
...
use _PhpScoper3fe455fa007d\PhpCsFixer\Differ\NullDiffer;
use _PhpScoper3fe455fa007d\PhpCsFixer\Differ\SebastianBergmannDiffer;
use _PhpScoper3fe455fa007d\PhpCsFixer\Differ\UnifiedDiffer;
use _PhpScoper3fe455fa007d\PhpCsFixer\Finder;
...
use _PhpScoper3fe455fa007d\PhpCsFixer\Report\ReporterFactory;
use _PhpScoper3fe455fa007d\PhpCsFixer\Report\ReporterInterface;
use _PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSet;
...
final class ConfigurationResolver
{
     ...
     * @param string $cwd
     */
    public function __construct(\_PhpScoper3fe455fa007d\PhpCsFixer\ConfigInterface $config, array $options, $cwd, \_PhpScoper3fe455fa007d\PhpCsFixer\ToolInfoInterface $toolInfo)
    {
        $this->cwd = $cwd;
    ...
    public function getCacheManager()
    {
        if (null === $this->cacheManager) {
            ...
            if (null === $cacheFile) {
                $this->cacheManager = new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\NullCacheManager();
            } else {
                $this->cacheManager = new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\FileCacheManager(new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\FileHandler($cacheFile), new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\Signature(\PHP_VERSION, $this->toolInfo->getVersion(), $this->getConfig()->getIndent(), $this->getConfig()->getLineEnding(), $this->getRules()), $this->isDryRun(), $this->getDirectory());
            }
        }
    ...
    public function getConfig()
    {
        if (null === $this->config) {
            foreach ($this->computeConfigFiles() as $configFile) {
                ...
                if (!$config instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigInterface) {
                    throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('The config file: "%s" does not return a "PhpCsFixer\\ConfigInterface" instance. Got: "%s".', $configFile, \is_object($config) ? \get_class($config) : \gettype($config)));
                }
                $this->config = $config;
    ...
    public function getDiffer()
    {
        if (null === $this->differ) {
            $mapper = ['null' => static function () {
                return new \_PhpScoper3fe455fa007d\PhpCsFixer\Differ\NullDiffer();
            }, 'sbd' => static function () {
                return new \_PhpScoper3fe455fa007d\PhpCsFixer\Differ\SebastianBergmannDiffer();
            }, 'udiff' => static function () {
                return new \_PhpScoper3fe455fa007d\PhpCsFixer\Differ\UnifiedDiffer();
            }];
            ...
            if (!\is_string($option)) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('"diff-format" must be a string, "%s" given.', \gettype($option)));
            }
            if (\is_subclass_of($option, \_PhpScoper3fe455fa007d\PhpCsFixer\Differ\DifferInterface::class)) {
                $this->differ = new $option();
            } elseif (!isset($mapper[$option])) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('"diff-format" must be any of "%s", got "%s".', \implode('", "', \array_keys($mapper)), $option));
            } else {
    ...
    public function getDirectory()
    {
        if (null === $this->directory) {
            ...
            $this->directory = new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\Directory(\dirname($absolutePath));
        }
    ...
    {
        if (null === $this->fixers) {
            $this->fixers = $this->createFixerFactory()->useRuleSet($this->getRuleSet())->setWhitespacesConfig(new \_PhpScoper3fe455fa007d\PhpCsFixer\WhitespacesFixerConfig($this->config->getIndent(), $this->config->getLineEnding()))->getFixers();
            if (\false === $this->getRiskyAllowed()) {
                $riskyFixers = \array_map(static function (\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer) {
                    return $fixer->getName();
                }, \array_filter($this->fixers, static function (\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer) {
                    return $fixer->isRisky();
                }));
                if (\count($riskyFixers)) {
                    throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('The rules contain risky fixers (%s), but they are not allowed to run. Perhaps you forget to use --allow-risky=yes option?', \implode('", "', $riskyFixers)));
                }
            }
        }
        return $this->fixers;
    ...
    {
        if (null === $this->linter) {
            $this->linter = new \_PhpScoper3fe455fa007d\PhpCsFixer\Linter\Linter($this->getConfig()->getPhpExecutable());
        }
        return $this->linter;
    }
    /**
        ...
        if (null === $this->path) {
            ...
            if (1 === \count($this->options['path']) && '-' === $this->options['path'][0]) {
                ...
                $this->path = \array_map(static function ($rawPath) use($cwd, $filesystem) {
                    $path = \trim($rawPath);
                    if ('' === $path) {
                        throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException("Invalid path: \"{$rawPath}\".");
                    }
                    $absolutePath = $filesystem->isAbsolutePath($path) ? $path : $cwd . \DIRECTORY_SEPARATOR . $path;
                    if (!\file_exists($absolutePath)) {
                        throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('The path "%s" is not readable.', $path));
                    }
                    return $absolutePath;
                }, $this->options['path']);
            }
    ...
    public function getProgress()
    {
        if (null === $this->progress) {
            if (\_PhpScoper3fe455fa007d\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $this->options['verbosity'] && 'txt' === $this->getFormat()) {
                ...
                if (null === $progressType) {
                    ...
                    throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('The progress type "%s" is not defined, supported are "%s".', $progressType, \implode('", "', $progressTypes)));
    ...
    public function getReporter()
    {
        if (null === $this->reporter) {
            $reporterFactory = \_PhpScoper3fe455fa007d\PhpCsFixer\Report\ReporterFactory::create();
            ...
            try {
                ...
                \sort($formats);
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('The format "%s" is not defined, supported are "%s".', $format, \implode('", "', $formats)));
            }
        }
    ...
    private function computeConfigFiles()
    {
        ...
        if (null !== $configFile) {
            if (\false === \file_exists($configFile) || \false === \is_readable($configFile)) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('Cannot read config file "%s".', $configFile));
            }
            return [$configFile];
        }
        $path = $this->getPath();
        if ($this->isStdIn() || 0 === \count($path)) {
            ...
            throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException('For multiple paths config parameter is required.');
    ...
    private function getRuleSet()
    {
        if (null === $this->ruleSet) {
            ...
            $this->ruleSet = new \_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSet($rules);
        }
        return $this->ruleSet;
    }
    /**
    ...
    private function parseRules()
    {
        ...
        if ('' === $rules) {
            throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException('Empty rules value is not allowed.');
        }
        if ('{' === $rules[0]) {
            $rules = \json_decode($rules, \true);
            if (\JSON_ERROR_NONE !== \json_last_error()) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('Invalid JSON rules input: "%s".', \json_last_error_msg()));
            }
            return $rules;
        }
        $rules = [];
        foreach (\explode(',', $this->options['rules']) as $rule) {
            $rule = \trim($rule);
            if ('' === $rule) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException('Empty rule name is not allowed.');
            }
            if ('-' === $rule[0]) {
    ...
    private function validateRules(array $rules)
    {
        ...
        foreach ($rules as $key => $value) {
            if (\is_int($key)) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('Missing value for "%s" rule/set.', $value));
            }
            $ruleSet[$key] = \true;
        }
        $ruleSet = new \_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSet($ruleSet);
        /** @var string[] $configuredFixers */
        $configuredFixers = \array_keys($ruleSet->getRules());
        $fixers = $this->createFixerFactory()->getFixers();
        /** @var string[] $availableFixers */
        $availableFixers = \array_map(static function (\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer) {
            return $fixer->getName();
        }, $fixers);
        $unknownFixers = \array_diff($configuredFixers, $availableFixers);
        if (\count($unknownFixers)) {
            $matcher = new \_PhpScoper3fe455fa007d\PhpCsFixer\WordMatcher($availableFixers);
            ...
            throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\substr($message, 0, -2) . '.');
        }
        foreach ($fixers as $fixer) {
            $fixerName = $fixer->getName();
            if (isset($rules[$fixerName]) && $fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\DeprecatedFixerInterface) {
                $successors = $fixer->getSuccessorsNames();
                $messageEnd = [] === $successors ? \sprintf(' and will be removed in version %d.0.', \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Application::getMajorVersion() + 1) : \sprintf('. Use %s instead.', \str_replace('`', '"', \_PhpScoper3fe455fa007d\PhpCsFixer\Utils::naturalLanguageJoinWithBackticks($successors)));
    ...
    private function resolveFinder()
    {
        ...
        if ($this->isStdIn()) {
            return new \ArrayIterator([new \_PhpScoper3fe455fa007d\PhpCsFixer\StdinFileInfo()]);
        }
        $modes = [self::PATH_MODE_OVERRIDE, self::PATH_MODE_INTERSECTION];
        if (!\in_array($this->options['path-mode'], $modes, \true)) {
            throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('The path-mode "%s" is not defined, supported are "%s".', $this->options['path-mode'], \implode('", "', $modes)));
        }
        ...
        if ($isIntersectionPathMode) {
            if (null === $nestedFinder) {
                throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException('Cannot create intersection with not-fully defined Finder in configuration file.');
            }
        ...
        if ($currentFinder instanceof \_PhpScoper3fe455fa007d\Symfony\Component\Finder\Finder && null === $nestedFinder) {
        ...
        return \_PhpScoper3fe455fa007d\PhpCsFixer\Finder::create()->in($pathsByType['dir'])->append($pathsByType['file']);
    }
    /**
    ...
    private function setOption($name, $value)
    {
        if (!\array_key_exists($name, $this->options)) {
            throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('Unknown option name: "%s".', $name));
        }
    ...
    private function resolveOptionBooleanValue($optionName)
    {
        ...
        if (!\is_string($value)) {
            throw new \_PhpScoper3fe455fa007d\PhpCsFixer\ConfigurationException\InvalidConfigurationException(\sprintf('Expected boolean or string value for option "%s".', $optionName));
        }
        if ('yes' === $value) {
            return \true;
        }
