...
final class Preg
{
    ...
    private static function newPregException($error, $method, array $patterns)
    {
        foreach ($patterns as $pattern) {
            ...
            return new \_PhpScoper3fe455fa007d\PhpCsFixer\PregException(\sprintf('%s(): Invalid PCRE pattern "%s": %s (version: %s)', $method, $pattern, $message, \PCRE_VERSION), $code);
        }
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\PregException(\sprintf('Error occurred when calling %s.', $method), $error);
    }
}
