...
final class TokensAnalyzer
{
     ...
     */
    private $tokens;
    public function __construct(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $this->tokens = $tokens;
    }
    /**
    ...
    {
        return $this->tokens[$index]->isGivenKind([\T_ARRAY, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_OPEN]);
    }
    /**
    ...
    public function isArrayMultiLine($index)
    {
        ...
        $endIndex = $tokens[$index]->equals('(') ? $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $index) : $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_ARRAY_SQUARE_BRACE, $index);
    ...
    public function isLambda($index)
    {
        ...
        if ($startParenthesisToken->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_RETURN_REF)) {
    ...
    public function isConstantInvocation($index)
    {
        ...
        if ($this->tokens[$nextIndex]->equalsAny(['(', '{']) || $this->tokens[$nextIndex]->isGivenKind([\T_AS, \T_DOUBLE_COLON, \T_ELLIPSIS, \T_NS_SEPARATOR, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_RETURN_REF, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_TYPE_ALTERNATION, \T_VARIABLE])) {
            return \false;
        }
        $prevIndex = $this->tokens->getPrevMeaningfulToken($index);
        if ($this->tokens[$prevIndex]->isGivenKind([\T_AS, \T_CLASS, \T_CONST, \T_DOUBLE_COLON, \T_FUNCTION, \T_GOTO, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_OPEN, \T_INTERFACE, \T_OBJECT_OPERATOR, \T_TRAIT, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_TYPE_COLON])) {
            return \false;
        }
        while ($this->tokens[$prevIndex]->isGivenKind([\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR, \T_NS_SEPARATOR, \T_STRING])) {
            $prevIndex = $this->tokens->getPrevMeaningfulToken($prevIndex);
        }
        if ($this->tokens[$prevIndex]->isGivenKind([\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_CONST_IMPORT, \T_EXTENDS, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_FUNCTION_IMPORT, \T_IMPLEMENTS, \T_INSTANCEOF, \T_INSTEADOF, \T_NAMESPACE, \T_NEW, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_NULLABLE_TYPE, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_TYPE_COLON, \T_USE, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_USE_TRAIT])) {
            return \false;
        }
        // `FOO & $bar` could be:
        ...
        if ($this->tokens[$prevIndex]->equals(',')) {
            ...
            while ($this->tokens[$checkIndex]->equalsAny([',', [\T_AS], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR], [\T_NS_SEPARATOR], [\T_STRING]])) {
                $checkIndex = $this->tokens->getPrevMeaningfulToken($checkIndex);
            }
            if ($this->tokens[$checkIndex]->isGivenKind([\T_EXTENDS, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_OPEN, \T_IMPLEMENTS, \T_USE, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_USE_TRAIT])) {
                return \false;
            }
        }
    ...
    {
        static $allowedPrevToken = [']', [\T_STRING], [\T_VARIABLE], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_PROP_BRACE_CLOSE], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_VAR_BRACE_CLOSE]];
        $tokens = $this->tokens;
        $token = $tokens[$index];
    ...
    public function isUnaryPredecessorOperator($index)
    {
        ...
        static $potentialBinaryOperator = ['+', '-', '&', [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_RETURN_REF]];
        ...
        if (null === $disallowedPrevTokens) {
            $disallowedPrevTokens = [']', '}', ')', '"', '`', [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_CLOSE], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_PROP_BRACE_CLOSE], [\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_VAR_BRACE_CLOSE], [\T_CLASS_C], [\T_CONSTANT_ENCAPSED_STRING], [\T_DEC], [\T_DIR], [\T_DNUMBER], [\T_FILE], [\T_FUNC_C], [\T_INC], [\T_LINE], [\T_LNUMBER], [\T_METHOD_C], [\T_NS_C], [\T_STRING], [\T_TRAIT_C], [\T_VARIABLE]];
        }
        $tokens = $this->tokens;
        $token = $tokens[$index];
    ...
    public function isBinaryOperator($index)
    {
        ...
        if (null === $arrayOperators) {
            $arrayOperators = [
                ...
                \T_XOR_EQUAL => \true,
                // ^=
                \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_TYPE_ALTERNATION => \true,
            ];
            if (\defined('T_SPACESHIP')) {
                $arrayOperators[\T_SPACESHIP] = \true;
                // <=>
            }
            if (\defined('T_COALESCE')) {
                $arrayOperators[\T_COALESCE] = \true;
                // ??
            }
            if (\defined('T_COALESCE_EQUAL')) {
                $arrayOperators[\T_COALESCE_EQUAL] = \true;
                // ??=
            }
        }
        $tokens = $this->tokens;
    ...
    public function isWhilePartOfDoWhile($index)
    {
        ...
        $startIndex = $tokens->findBlockStart(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $endIndex);
    ...
    private function findClassyElements($classIndex, $index)
    {
        ...
        for ($count = \count($this->tokens); $index < $count; ++$index) {
            ...
            if (0 === $bracesLevel && $token->isGivenKind(\T_VARIABLE)) {
                $elements[$index] = ['token' => $token, 'type' => 'property', 'classIndex' => $classIndex];
                continue;
            }
            if ($token->isGivenKind(\T_FUNCTION)) {
                $elements[$index] = ['token' => $token, 'type' => 'method', 'classIndex' => $classIndex];
            } elseif ($token->isGivenKind(\T_CONST)) {
                $elements[$index] = ['token' => $token, 'type' => 'const', 'classIndex' => $classIndex];
            }
        }
