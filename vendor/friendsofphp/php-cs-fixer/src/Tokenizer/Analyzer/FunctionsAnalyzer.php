...
final class FunctionsAnalyzer
{
     ...
     * @return bool
     */
    public function isGlobalFunctionCall(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        if ($tokens[$prevIndex]->isGivenKind([\T_DOUBLE_COLON, \T_FUNCTION, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR, \T_NEW, \T_OBJECT_OPERATOR, \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_RETURN_REF, \T_STRING])) {
            return \false;
        }
        ...
        $namespaceAnalyzer = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\NamespacesAnalyzer();
     ...
     */
    public function getFunctionArguments(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $methodIndex)
    {
        $argumentsStart = $tokens->getNextTokenOfKind($methodIndex, ['(']);
        $argumentsEnd = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $argumentsStart);
        $argumentAnalyzer = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\ArgumentsAnalyzer();
        $arguments = [];
     ...
     */
    public function getFunctionReturnType(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $methodIndex)
    {
        $argumentsStart = $tokens->getNextTokenOfKind($methodIndex, ['(']);
        $argumentsEnd = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $argumentsStart);
        ...
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\Analysis\TypeAnalysis($type, $typeStartIndex, $typeEndIndex);
    }
    /**
     * @param int $index
     *
     * @return bool
     */
    public function isTheSameClassCall(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
    ...
    private function buildFunctionsAnalysis(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        if ($tokens->isTokenKindFound(\T_FUNCTION)) {
            ...
            for ($i = 0; $i < $end; ++$i) {
                ...
                if ($tokens[$i]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token::getClassyTokenKinds())) {
                    $i = $tokens->getNextTokenOfKind($i, ['(', '{']);
                    if ($tokens[$i]->equals('(')) {
                        // anonymous class
                        $i = $tokens->findBlockEnd(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $i);
                        $i = $tokens->getNextTokenOfKind($i, ['{']);
                    }
                ...
                if ($tokens[$i]->isGivenKind(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\CT::T_RETURN_REF)) {
                    $i = $tokens->getNextMeaningfulToken($i);
                }
        ...
        $namespaceUsesAnalyzer = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Analyzer\NamespaceUsesAnalyzer();
