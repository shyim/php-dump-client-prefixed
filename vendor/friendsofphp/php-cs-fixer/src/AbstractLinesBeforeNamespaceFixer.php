 ...
 * @internal
 */
abstract class AbstractLinesBeforeNamespaceFixer extends \_PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer implements \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    /**
     ...
     */
    protected function fixLinesBeforeNamespace(\_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $expectedMin, $expectedMax)
    {
        ...
        if (0 === $expectedMax) {
            ...
            if ($newlineInOpening) {
                $tokens[$openingTokenIndex] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_OPEN_TAG, \rtrim($openingToken->getContent()) . ' ']);
            }
            return;
        }
        ...
        if (null !== $openingToken) {
            ...
            $tokens[$openingTokenIndex] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_OPEN_TAG, $newContent]);
        ...
        if ($previous->isWhitespace()) {
            ...
            $tokens[$previousIndex] = new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, \str_repeat($lineEnding, $newlinesForWhitespaceToken) . \substr($previous->getContent(), \strrpos($previous->getContent(), "\n") + 1)]);
        } else {
            // Add a new whitespace token
            $tokens->insertAt($index, new \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, \str_repeat($lineEnding, $newlinesForWhitespaceToken)]));
        }
    }
}
