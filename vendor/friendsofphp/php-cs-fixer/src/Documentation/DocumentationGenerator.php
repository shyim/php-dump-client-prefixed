 ...
 */
namespace _PhpScoper3fe455fa007d\PhpCsFixer\Documentation;

use _PhpScoper3fe455fa007d\PhpCsFixer\AbstractFixer;
use _PhpScoper3fe455fa007d\PhpCsFixer\Console\Command\HelpCommand;
use _PhpScoper3fe455fa007d\PhpCsFixer\Diff\GeckoPackages\DiffOutputBuilder\UnifiedDiffOutputBuilder;
use _PhpScoper3fe455fa007d\PhpCsFixer\Diff\v2_0\Differ;
use _PhpScoper3fe455fa007d\PhpCsFixer\Fixer\Basic\Psr0Fixer;
...
use _PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\DeprecatedFixerOptionInterface;
...
use _PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSetDescriptionInterface;
...
final class DocumentationGenerator
{
    ...
    {
        $this->differ = new \_PhpScoper3fe455fa007d\PhpCsFixer\Diff\v2_0\Differ(new \_PhpScoper3fe455fa007d\PhpCsFixer\Diff\GeckoPackages\DiffOutputBuilder\UnifiedDiffOutputBuilder(['fromFile' => 'Original', 'toFile' => 'New']));
    ...
    public function generateFixersDocumentationIndex(array $fixers)
    {
        ...
        \usort($fixers, function (\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $a, \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $b) {
...
RST;
        ...
        foreach ($fixers as $fixer) {
            $namespace = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/^.*\\\\(.+)\\\\.+Fixer$/', '$1', \get_class($fixer));
            if (isset($overrideGroups[$namespace])) {
                $group = $overrideGroups[$namespace];
            } else {
                $group = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/(?<=[[:lower:]])(?=[[:upper:]])/', ' ', $namespace);
            }
...
RST;
     ...
     * @return string
     */
    public function getFixerDocumentationFilePath(\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer)
    {
        return $this->getFixersDocumentationDirectoryPath() . '/' . \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replaceCallback('/^.*\\\\(.+)\\\\(.+)Fixer$/', function (array $matches) {
            return \_PhpScoper3fe455fa007d\PhpCsFixer\Utils::camelCaseToUnderscore($matches[1]) . '/' . \_PhpScoper3fe455fa007d\PhpCsFixer\Utils::camelCaseToUnderscore($matches[2]);
        }, \get_class($fixer)) . '.rst';
    }
    /**
     * @return string
     */
    public function getFixerDocumentationFileRelativePath(\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer)
    {
        return \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('#^' . \preg_quote($this->getFixersDocumentationDirectoryPath(), '#') . '/#', '', $this->getFixerDocumentationFilePath($fixer));
    }
    /**
     * @return string
     */
    public function generateFixerDocumentation(\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer)
    {
        $name = $fixer->getName();
        $title = "Rule ``{$name}``";
        ...
        if ($fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\DeprecatedFixerInterface) {
            ...
            if ([] !== $alternatives) {
                $doc .= $this->toRst(\sprintf("\n\nYou should use %s instead.", \_PhpScoper3fe455fa007d\PhpCsFixer\Utils::naturalLanguageJoinWithBackticks($alternatives)), 3);
            }
        }
...
RST;
            ...
            foreach ($configurationDefinition->getOptions() as $option) {
                ...
                if ($option instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\DeprecatedFixerOptionInterface) {
                ...
                if (null !== $allowed) {
                    foreach ($allowed as &$value) {
                        if ($value instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerConfiguration\AllowedValueSubset) {
                            $value = 'a subset of ``' . \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Command\HelpCommand::toString($value->getAllowedValues()) . '``';
                        } else {
                            $value = '``' . \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Command\HelpCommand::toString($value) . '``';
                        }
                    }
                } else {
                ...
                if ($option->hasDefault()) {
                    $default = \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Command\HelpCommand::toString($option->getDefault());
...
RST;
            foreach ($samples as $index => $sample) {
                ...
                if ($fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurableFixerInterface) {
                    if (null === $sample->getConfiguration()) {
                        ...
                        $doc .= \sprintf("\n\nWith configuration: ``%s``.", \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Command\HelpCommand::toString($sample->getConfiguration()));
                    }
                }
        ...
        $ruleSetConfigs = [];
        foreach (\_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSets::getSetDefinitionNames() as $set) {
            $ruleSet = new \_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSet([$set => \true]);
...
RST;
                ...
                } elseif ($fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface) {
     ...
     * @return string
     */
    public function generateRuleSetsDocumentation(\_PhpScoper3fe455fa007d\PhpCsFixer\RuleSet\RuleSetDescriptionInterface $definition, array $fixers)
    {
        $fixerNames = [];
        foreach ($fixers as $fixer) {
        ...
        if (\count($rules) < 1) {
            ...
            foreach ($rules as $rule => $config) {
                if ('@' === $rule[0]) {
                    ...
                    $path = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('#^' . \preg_quote($this->getFixersDocumentationDirectoryPath(), '#') . '/#', './../rules/', $this->getFixerDocumentationFilePath($fixerNames[$rule]));
                    $doc .= "\n- `{$rule} <{$path}>`_";
                }
                if (!\is_bool($config)) {
                    $doc .= "\n  config:\n  ``" . \_PhpScoper3fe455fa007d\PhpCsFixer\Console\Command\HelpCommand::toString($config) . '``';
...
RST;
     ...
     * @return string
     */
    private function generateSampleDiff(\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer, \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\CodeSampleInterface $sample, $sampleNumber, $ruleName)
    {
        if ($sample instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\VersionSpecificCodeSampleInterface && !$sample->isSuitableFor(\PHP_VERSION_ID)) {
            ...
            if (\false !== $existingFile) {
                \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::match("/\\RExample #{$sampleNumber}\\R.+?(?<diff>\\R\\.\\. code-block:: diff\\R\\R.*?)\\R(?:\\R\\S|\$)/s", $existingFile, $matches);
...
RST;
        ...
        $file = $sample instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\FixerDefinition\FileSpecificCodeSampleInterface ? $sample->getSplFileInfo() : new \_PhpScoper3fe455fa007d\PhpCsFixer\StdinFileInfo();
        if ($fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\ConfigurableFixerInterface) {
            $configuration = $sample->getConfiguration();
            if (null === $configuration) {
                $configuration = [];
            }
            if ($fixer instanceof \_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\Basic\Psr0Fixer && isset($configuration['dir']) && 0 === \strpos($configuration['dir'], './')) {
        ...
        $diff = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/\\r/', '^M', $diff);
        $diff = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/^ $/m', '', $diff);
        $diff = \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/\\n$/', '', $diff);
        return <<<RST
...
RST;
    ...
    {
        $string = \wordwrap(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace('/(?<!`)(`.*?`)(?!`)/', '`$1`', $string), 80 - $indent);
        if (0 !== $indent) {
