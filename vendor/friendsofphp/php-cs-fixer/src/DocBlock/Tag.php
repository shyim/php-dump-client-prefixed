...
class Tag
{
     ...
     */
    public function __construct(\_PhpScoper3fe455fa007d\PhpCsFixer\DocBlock\Line $line)
    {
        $this->line = $line;
    }
    /**
    ...
    {
        if (null === $this->name) {
            \_PhpScoper3fe455fa007d\PhpCsFixer\Preg::matchAll('/@[a-zA-Z0-9_-]+(?=\\s|$)/', $this->line->getContent(), $matches);
    ...
    public function setName($name)
    {
        ...
        $this->line->setContent(\_PhpScoper3fe455fa007d\PhpCsFixer\Preg::replace("/@{$current}/", "@{$name}", $this->line->getContent(), 1));
        $this->name = $name;
    }
    /**
