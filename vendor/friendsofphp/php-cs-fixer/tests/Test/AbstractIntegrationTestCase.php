 ...
 * @internal
 */
abstract class AbstractIntegrationTestCase extends \_PhpScoper3fe455fa007d\PhpCsFixer\Tests\TestCase
{
    ...
    public static function doSetUpBeforeClass()
    {
        ...
        self::$fileRemoval = new \_PhpScoper3fe455fa007d\PhpCsFixer\FileRemoval();
        self::$fileRemoval->observe($tmpFile);
        if (!\is_file($tmpFile)) {
            $dir = \dirname($tmpFile);
            if (!\is_dir($dir)) {
                $fs = new \_PhpScoper3fe455fa007d\Symfony\Component\Filesystem\Filesystem();
                $fs->mkdir($dir, 0766);
            }
        }
    }
     ...
     * @see doTest()
     */
    public function testIntegration(\_PhpScoper3fe455fa007d\PhpCsFixer\Tests\Test\IntegrationCase $case)
    {
        $this->doTest($case);
    }
    /**
    ...
    public function provideIntegrationCases()
    {
        ...
        foreach (\_PhpScoper3fe455fa007d\Symfony\Component\Finder\Finder::create()->files()->in($fixturesDir) as $file) {
    ...
    {
        return new \_PhpScoper3fe455fa007d\PhpCsFixer\Tests\Test\IntegrationCaseFactory();
    }
    /**
     ...
     */
    protected function doTest(\_PhpScoper3fe455fa007d\PhpCsFixer\Tests\Test\IntegrationCase $case)
    {
        ...
        if (\false === @\file_put_contents($tmpFile, $input)) {
            throw new \_PhpScoper3fe455fa007d\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Failed to write to tmp. file "%s".', $tmpFile));
        }
        $errorsManager = new \_PhpScoper3fe455fa007d\PhpCsFixer\Error\ErrorsManager();
        $fixers = static::createFixers($case);
        $runner = new \_PhpScoper3fe455fa007d\PhpCsFixer\Runner\Runner(new \ArrayIterator([new \SplFileInfo($tmpFile)]), $fixers, new \_PhpScoper3fe455fa007d\PhpCsFixer\Differ\SebastianBergmannDiffer(), null, $errorsManager, $this->linter, \false, new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\NullCacheManager());
        ...
        if (1 < \count($fixers)) {
            ...
            $runner = new \_PhpScoper3fe455fa007d\PhpCsFixer\Runner\Runner(new \ArrayIterator([new \SplFileInfo($tmpFile)]), \array_reverse($fixers), new \_PhpScoper3fe455fa007d\PhpCsFixer\Differ\SebastianBergmannDiffer(), null, $errorsManager, $this->linter, \false, new \_PhpScoper3fe455fa007d\PhpCsFixer\Cache\NullCacheManager());
            \_PhpScoper3fe455fa007d\PhpCsFixer\Tokenizer\Tokens::clearCache();
            $runner->fix();
        ...
        $this->testIntegration(new \_PhpScoper3fe455fa007d\PhpCsFixer\Tests\Test\IntegrationCase($case->getFileName(), $case->getTitle() . ' "--EXPECT-- part run"', $case->getSettings(), $case->getRequirements(), $case->getConfig(), $case->getRuleset(), $case->getExpectedCode(), null));
    }
    /**
     * @param string $fixedInputCode
     * @param string $fixedInputCodeWithReversedFixers
     */
    protected static function assertRevertedOrderFixing(\_PhpScoper3fe455fa007d\PhpCsFixer\Tests\Test\IntegrationCase $case, $fixedInputCode, $fixedInputCodeWithReversedFixers)
    {
        ...
        if ($fixedInputCode !== $fixedInputCodeWithReversedFixers) {
            static::assertGreaterThan(1, \count(\array_unique(\array_map(static function (\_PhpScoper3fe455fa007d\PhpCsFixer\Fixer\FixerInterface $fixer) {
     ...
     */
    private static function createFixers(\_PhpScoper3fe455fa007d\PhpCsFixer\Tests\Test\IntegrationCase $case)
    {
        $config = $case->getConfig();
        return \_PhpScoper3fe455fa007d\PhpCsFixer\FixerFactory::create()->registerBuiltInFixers()->useRuleSet($case->getRuleset())->setWhitespacesConfig(new \_PhpScoper3fe455fa007d\PhpCsFixer\WhitespacesFixerConfig($config['indent'], $config['lineEnding']))->getFixers();
    }
    /**
     * @param Error[] $errors
     *
    ...
    private function getLinter()
    {
        ...
        if (null === $linter) {
            if (\getenv('SKIP_LINT_TEST_CASES')) {
                ...
                $linterProphecy->lintFile(\_PhpScoper3fe455fa007d\Prophecy\Argument::type('string'))->willReturn($this->prophesize(\_PhpScoper3fe455fa007d\PhpCsFixer\Linter\LintingResultInterface::class)->reveal());
