 ...
 */
namespace _PhpScoper3fe455fa007d\PhpCsFixer\Diff\v2_0\Output;

/**
 * Builds a diff string representation in unified diff format in chunks.
 */
final class UnifiedDiffOutputBuilder extends \_PhpScoper3fe455fa007d\PhpCsFixer\Diff\v2_0\Output\AbstractChunkOutputBuilder
{
    /**
     * @var string
     */
    ...
    private function writeDiffChunked($output, array $diff, array $old)
    {
        ...
        $toStart = 0;
        if (\count($old)) {
            // no common parts, list all diff entries
            \reset($old);
            ...
            do {
                ...
                // calculate number of non-change lines in the common part
                $fromStart += $commonLength;
                $toStart += $commonLength;
            } while (\false !== \next($old));
            \end($old);
            // short cut for finding possible last `change entry`
            $tmp = \key($old);
            \reset($old);
        ...
        if ($start < $upperLimit - 1) {
            // check for trailing (non) diff entries
            do {
                --$upperLimit;
    ...
    private function writeChunk($output, array $diff, $diffStartIndex, $diffEndIndex, $fromStart, $fromRange, $toStart, $toRange)
    {
