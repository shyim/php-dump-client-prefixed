...
final class Tokenizer
{
     ...
     */
    public function tokenize(string $string) : \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Cursor
    {
        $tokens = [];
        ...
        while ($currentLength) {
            ...
            if ($oldStringLen <= $currentLength) {
                $tokens[] = new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_ERROR, $string);
                return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Cursor($tokens);
            }
     ...
     */
    private function createNextToken(string $string, ?\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token $previous = null) : \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token
    {
        $matches = [];
        // Whitespace
        if (\preg_match('/^\\s+/', $string, $matches)) {
            return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_WHITESPACE, $matches[0]);
        }
        // Comment
        if ($string[0] === '#' || (isset($string[1]) && ($string[0] === '-' && $string[1] === '-') || isset($string[1]) && $string[0] === '/' && $string[1] === '*')) {
            ...
            if ($string[0] === '-' || $string[0] === '#') {
                $last = \strpos($string, "\n");
                $type = \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_COMMENT;
            } else {
                // Comment until closing comment tag
                $pos = \strpos($string, '*/', 2);
                \assert($pos !== \false);
                $last = $pos + 2;
                $type = \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_BLOCK_COMMENT;
            }
            if ($last === \false) {
                $last = \strlen($string);
            }
            return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token($type, \substr($string, 0, $last));
        }
        // Quoted String
        if ($string[0] === '"' || $string[0] === '\'' || $string[0] === '`' || $string[0] === '[') {
            return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token($string[0] === '`' || $string[0] === '[' ? \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_BACKTICK_QUOTE : \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_QUOTE, $this->getQuotedString($string));
        }
        // User-defined Variable
        if (($string[0] === '@' || $string[0] === ':') && isset($string[1])) {
            $value = null;
            $type = \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_VARIABLE;
            ...
            if ($value !== null) {
                return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token($type, $value);
            }
        }
        // Number (decimal, binary, or hex)
        if (\preg_match('/^([0-9]+(\\.[0-9]+)?|0x[0-9a-fA-F]+|0b[01]+)($|\\s|"\'`|' . $this->regexBoundaries . ')/', $string, $matches)) {
            return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_NUMBER, $matches[1]);
        }
        // Boundary Character (punctuation and symbols)
        if (\preg_match('/^(' . $this->regexBoundaries . ')/', $string, $matches)) {
            return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_BOUNDARY, $matches[1]);
        }
        ...
        if (!$previous || $previous->value() !== '.') {
            ...
            if (\preg_match('/^(' . $this->regexReservedToplevel . ')($|\\s|' . $this->regexBoundaries . ')/', $upper, $matches)) {
                return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED_TOPLEVEL, \substr($string, 0, \strlen($matches[1])));
            }
            // Newline Reserved Word
            if (\preg_match('/^(' . $this->regexReservedNewline . ')($|\\s|' . $this->regexBoundaries . ')/', $upper, $matches)) {
                return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED_NEWLINE, \substr($string, 0, \strlen($matches[1])));
            }
            // Other Reserved Word
            if (\preg_match('/^(' . $this->regexReserved . ')($|\\s|' . $this->regexBoundaries . ')/', $upper, $matches)) {
                return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED, \substr($string, 0, \strlen($matches[1])));
            }
        }
        ...
        if (\preg_match('/^(' . $this->regexFunction . '[(]|\\s|[)])/', $upper, $matches)) {
            return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED, \substr($string, 0, \strlen($matches[1]) - 1));
        }
        // Non reserved word
        \preg_match('/^(.*?)($|\\s|["\'`]|' . $this->regexBoundaries . ')/', $string, $matches);
        return new \_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token(\_PhpScopereaa8bfd44f12\Doctrine\SqlFormatter\Token::TOKEN_TYPE_WORD, $matches[1]);
    }
    /**
