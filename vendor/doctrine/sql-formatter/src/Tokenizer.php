...
final class Tokenizer
{
    ...
    public function __construct()
    {
        ...
        $this->regexBoundaries = '(' . \implode('|', $this->quoteRegex($this->boundaries)) . ')';
        $this->regexReserved = '(' . \implode('|', $this->quoteRegex($this->reserved)) . ')';
        $this->regexReservedToplevel = \str_replace(' ', '\\s+', '(' . \implode('|', $this->quoteRegex($this->reservedToplevel)) . ')');
        $this->regexReservedNewline = \str_replace(' ', '\\s+', '(' . \implode('|', $this->quoteRegex($this->reservedNewline)) . ')');
     ...
     */
    public function tokenize(string $string) : \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Cursor
    {
        $tokens = [];
        ...
        while ($currentLength) {
            ...
            if ($oldStringLen <= $currentLength) {
                $tokens[] = new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_ERROR, $string);
                return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Cursor($tokens);
            }
     ...
     */
    private function createNextToken(string $string, ?\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token $previous = null) : \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token
    {
        $matches = [];
        // Whitespace
        if (\preg_match('/^\\s+/', $string, $matches)) {
            return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_WHITESPACE, $matches[0]);
        }
        // Comment
        if ($string[0] === '#' || (isset($string[1]) && ($string[0] === '-' && $string[1] === '-') || isset($string[1]) && $string[0] === '/' && $string[1] === '*')) {
            ...
            if ($string[0] === '-' || $string[0] === '#') {
                $last = \strpos($string, "\n");
                $type = \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_COMMENT;
            } else {
                // Comment until closing comment tag
                $pos = \strpos($string, '*/', 2);
                \assert($pos !== \false);
                $last = $pos + 2;
                $type = \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_BLOCK_COMMENT;
            }
            if ($last === \false) {
                $last = \strlen($string);
            }
            return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token($type, \substr($string, 0, $last));
        }
        // Quoted String
        if ($string[0] === '"' || $string[0] === '\'' || $string[0] === '`' || $string[0] === '[') {
            return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token($string[0] === '`' || $string[0] === '[' ? \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_BACKTICK_QUOTE : \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_QUOTE, $this->getQuotedString($string));
        }
        // User-defined Variable
        if (($string[0] === '@' || $string[0] === ':') && isset($string[1])) {
            $value = null;
            $type = \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_VARIABLE;
            ...
            if ($value !== null) {
                return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token($type, $value);
            }
        }
        // Number (decimal, binary, or hex)
        if (\preg_match('/^([0-9]+(\\.[0-9]+)?|0x[0-9a-fA-F]+|0b[01]+)($|\\s|"\'`|' . $this->regexBoundaries . ')/', $string, $matches)) {
            return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_NUMBER, $matches[1]);
        }
        // Boundary Character (punctuation and symbols)
        if (\preg_match('/^(' . $this->regexBoundaries . ')/', $string, $matches)) {
            return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_BOUNDARY, $matches[1]);
        }
        ...
        if (!$previous || $previous->value() !== '.') {
            ...
            if (\preg_match('/^(' . $this->regexReservedToplevel . ')($|\\s|' . $this->regexBoundaries . ')/', $upper, $matches)) {
                return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED_TOPLEVEL, \substr($string, 0, \strlen($matches[1])));
            }
            // Newline Reserved Word
            if (\preg_match('/^(' . $this->regexReservedNewline . ')($|\\s|' . $this->regexBoundaries . ')/', $upper, $matches)) {
                return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED_NEWLINE, \substr($string, 0, \strlen($matches[1])));
            }
            // Other Reserved Word
            if (\preg_match('/^(' . $this->regexReserved . ')($|\\s|' . $this->regexBoundaries . ')/', $upper, $matches)) {
                return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED, \substr($string, 0, \strlen($matches[1])));
            }
        }
        ...
        if (\preg_match('/^(' . $this->regexFunction . '[(]|\\s|[)])/', $upper, $matches)) {
            return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_RESERVED, \substr($string, 0, \strlen($matches[1]) - 1));
        }
        // Non reserved word
        \preg_match('/^(.*?)($|\\s|["\'`]|' . $this->regexBoundaries . ')/', $string, $matches);
        return new \_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token(\_PhpScoper3fe455fa007d\Doctrine\SqlFormatter\Token::TOKEN_TYPE_WORD, $matches[1]);
    }
    /**
