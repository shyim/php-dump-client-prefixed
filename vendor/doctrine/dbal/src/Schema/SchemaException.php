 ...
 * @psalm-immutable
 */
class SchemaException extends \_PhpScoper3fe455fa007d\Doctrine\DBAL\Exception
{
    ...
    {
        return new self(\sprintf('Invalid index-name %s given, has to be [a-zA-Z0-9_]', $indexName), self::INDEX_INVALID_NAME);
    }
    /**
     * @param string $indexName
     * @param string $table
    ...
    {
        return new self(\sprintf("Index '%s' does not exist on table '%s'.", $indexName, $table), self::INDEX_DOESNT_EXIST);
    }
    /**
     * @param string $indexName
     * @param string $table
    ...
    {
        return new self(\sprintf("An index with name '%s' was already defined on table '%s'.", $indexName, $table), self::INDEX_ALREADY_EXISTS);
    }
    /**
    ...
    {
        return new self(\sprintf("There exists no foreign key with the name '%s' on table '%s'.", $fkName, $table), self::FOREIGNKEY_DOESNT_EXIST);
    }
    /**
     * @return SchemaException
     */
    public static function namedForeignKeyRequired(\_PhpScoper3fe455fa007d\Doctrine\DBAL\Schema\Table $localTable, \_PhpScoper3fe455fa007d\Doctrine\DBAL\Schema\ForeignKeyConstraint $foreignKey)
    {
