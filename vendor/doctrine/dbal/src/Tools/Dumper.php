...
namespace _PhpScoper3fe455fa007d\Doctrine\DBAL\Tools;

use ArrayIterator;
use ArrayObject;
use DateTimeInterface;
use _PhpScoper3fe455fa007d\Doctrine\Common\Collections\Collection;
use _PhpScoper3fe455fa007d\Doctrine\Common\Persistence\Proxy;
use stdClass;
...
final class Dumper
{
    ...
    public static function export($var, int $maxDepth)
    {
        $return = null;
        $isObj = \is_object($var);
        if ($var instanceof \_PhpScoper3fe455fa007d\Doctrine\Common\Collections\Collection) {
            $var = $var->toArray();
        }
        if ($maxDepth === 0) {
        ...
        if ($var instanceof \_PhpScoper3fe455fa007d\Doctrine\Common\Persistence\Proxy) {
    ...
    private static function getClass($object) : string
    {
        ...
        if (!\class_exists(\_PhpScoper3fe455fa007d\Doctrine\Common\Persistence\Proxy::class)) {
            return $class;
        }
        $pos = \strrpos($class, '\\' . \_PhpScoper3fe455fa007d\Doctrine\Common\Persistence\Proxy::MARKER . '\\');
        if ($pos === \false) {
            return $class;
        }
        return \substr($class, $pos + \strlen(\_PhpScoper3fe455fa007d\Doctrine\Common\Persistence\Proxy::MARKER) + 2);
    }
}
