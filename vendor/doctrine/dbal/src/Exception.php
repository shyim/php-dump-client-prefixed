...
class Exception extends \Exception
{
    ...
    public static function invalidPlatformType($invalidPlatform) : self
    {
        if (\is_object($invalidPlatform)) {
            return new self(\sprintf("Option 'platform' must be a subtype of '%s', instance of '%s' given", \_PhpScoper3fe455fa007d\Doctrine\DBAL\Platforms\AbstractPlatform::class, \get_class($invalidPlatform)));
        }
        return new self(\sprintf("Option 'platform' must be an object and subtype of '%s'. Got '%s'", \_PhpScoper3fe455fa007d\Doctrine\DBAL\Platforms\AbstractPlatform::class, \gettype($invalidPlatform)));
    }
    /**
    ...
    {
        return new self("The given 'driverClass' " . $driverClass . ' has to implement the ' . \_PhpScoper3fe455fa007d\Doctrine\DBAL\Driver::class . ' interface.');
    }
    ...
    public static function typeNotRegistered(\_PhpScoper3fe455fa007d\Doctrine\DBAL\Types\Type $type) : self
    {
        return new self(\sprintf('Type of the class %s@%s is not registered.', \get_class($type), \spl_object_hash($type)));
    }
    public static function typeAlreadyRegistered(\_PhpScoper3fe455fa007d\Doctrine\DBAL\Types\Type $type) : self
    {
