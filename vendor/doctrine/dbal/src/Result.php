...
use _PhpScoper3fe455fa007d\Doctrine\DBAL\Exception\NoKeyValue;
use LogicException;
use Traversable;
...
class Result
{
     ...
     */
    public function __construct(\_PhpScoper3fe455fa007d\Doctrine\DBAL\Driver\Result $result, \_PhpScoper3fe455fa007d\Doctrine\DBAL\Connection $connection)
    {
        $this->result = $result;
    ...
    private function ensureHasKeyValue() : void
    {
        ...
        if ($columnCount < 2) {
            throw \_PhpScoper3fe455fa007d\Doctrine\DBAL\Exception\NoKeyValue::fromColumnCount($columnCount);
        }
    }
    /**
     ...
     * @return mixed
     */
    public function fetch(int $mode = \_PhpScoper3fe455fa007d\Doctrine\DBAL\FetchMode::ASSOCIATIVE)
    {
        if (\func_num_args() > 1) {
            throw new \LogicException('Only invocations with one argument are still supported by this legecy API.');
        }
        if ($mode === \_PhpScoper3fe455fa007d\Doctrine\DBAL\FetchMode::ASSOCIATIVE) {
            return $this->fetchAssociative();
        }
        if ($mode === \_PhpScoper3fe455fa007d\Doctrine\DBAL\FetchMode::NUMERIC) {
            return $this->fetchNumeric();
        }
        if ($mode === \_PhpScoper3fe455fa007d\Doctrine\DBAL\FetchMode::COLUMN) {
     ...
     * @return list<mixed>
     */
    public function fetchAll(int $mode = \_PhpScoper3fe455fa007d\Doctrine\DBAL\FetchMode::ASSOCIATIVE) : array
    {
        if (\func_num_args() > 1) {
            throw new \LogicException('Only invocations with one argument are still supported by this legecy API.');
        }
