...
class Timer
{
    ...
    private \_PhpScoper926b1169e332\PhpDumpClient\Message\Payload\TablePayload $table;
    private \_PhpScoper926b1169e332\PhpDumpClient\Client $client;
    private \_PhpScoper926b1169e332\PhpDumpClient\Message\Message $message;
    private string $title;
    public function __construct(string $title, \_PhpScoper926b1169e332\PhpDumpClient\Message\Message $message, \_PhpScoper926b1169e332\PhpDumpClient\Client $client)
    {
        $this->time = \microtime(\true);
        $this->memoryUsage = \memory_get_usage(\true);
        $this->peakMemoryUsage = \memory_get_peak_usage(\true);
        $this->table = new \_PhpScoper926b1169e332\PhpDumpClient\Message\Payload\TablePayload(['Time', 'Total Memory Usage', 'Peak Memory Usage']);
        $this->client = $client;
        $this->message = $message;
        $this->title = $title;
        $this->message->payload(new \_PhpScoper926b1169e332\PhpDumpClient\Message\Payload\HtmlPayload($title));
    ...
    public function stop() : \_PhpScoper926b1169e332\PhpDumpClient\Client
    {
        $this->track();
